...
impl<DI, M, RST> AsyncDrawTarget for Display<DI, M, RST>
where
    ...
    RST: OutputPin,
{
    ...
    async fn fill_contiguous_async<I>(&mut self, area: &Rectangle, colors: I) -> Result<(), Self::Error>
    where
        I: IntoIterator<Item = Self::Color>,
    {
        ...
        if &intersection == area {
            ...
            self.set_pixels(sx, sy, ex, ey, take_u32(colors, count)).await
        } else {
    ...
    async fn fill_solid_async(&mut self, area: &Rectangle, color: Self::Color) -> Result<(), Self::Error> {
        ...
        let ex = bottom_right.x as u16;
        let ey = bottom_right.y as u16;
        self.set_pixels(sx, sy, ex, ey, &mut colors).await
    }
}
